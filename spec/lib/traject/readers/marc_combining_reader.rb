...
describe Traject::MarcCombiningReader do
  ...
  describe '#each' do
    it 'merges MARC records when their 001 fields match' do
      ...
      expect(results.map { |x| x['001'].value }).to eq ['anotSplit1', 'anotSplit2', 'asplit1', 'asplit2', 'asplit3']
      ...
      expect(asplit1.fields('999').map { |x| x['a'] }).to eq ['A1 .B2 V.1', 'A1 .B2 V.2', 'A1 .B2 V.3', 'A1 .B2 V.4', 'A1 .B2 V.5']
    end
  end
end
