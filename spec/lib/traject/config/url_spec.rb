RSpec.describe 'Access config' do
  ...
  describe 'url_fulltext' do
    ...
    it 'adds url_fulltext fields to records with fulltext url(s) in docs' do
      ...
      expect(select_by_id('mult856and956')[field]).to eq ["http://www.sciencemag.org/", "http://www.jstor.org/journals/00368075.html", "http://www.sciencemag.org/archive/"]
  ...
  describe 'url field ordering' do
    ...
    it 'preserves field ordering from marc21 input to marc21 stored in record' do
      ...
      expect(select_by_id('fulltextAndRestricted1')['url_fulltext']).to eq ['http://first.org', 'http://second.org', 'http://restricted.org/first', 'http://restricted.org/second']

      expect(select_by_id('fulltextAndRestricted2')['url_restricted']).to eq ['http://restricted.org/first', 'http://restricted.org/second']
      expect(select_by_id('fulltextAndRestricted2')['url_fulltext']).to eq ['http://first.org', 'http://restricted.org/first', 'http://second.org', 'http://restricted.org/second']

      expect(select_by_id('fulltextAndRestricted3')['url_restricted']).to eq ['http://restricted.org/first', 'http://restricted.org/second']
      expect(select_by_id('fulltextAndRestricted3')['url_fulltext']).to eq ['http://restricted.org/first', 'http://first.org', 'http://second.org', 'http://restricted.org/second']

      expect(select_by_id('fulltextAndRestricted4')['url_restricted']).to eq ['http://restricted.org/first', 'http://restricted.org/second']
      expect(select_by_id('fulltextAndRestricted4')['url_fulltext']).to eq ['http://first.org', 'http://restricted.org/first', 'http://restricted.org/second', 'http://second.org']

      expect(select_by_id('fulltextAndSuppl1')['url_restricted']).to eq ['http://restricted.org/first', 'http://restricted.org/second']
      expect(select_by_id('fulltextAndSuppl1')['url_fulltext']).to eq ['http://www.fulltext.org/first', 'http://www.fulltext.org/second', 'http://restricted.org/first', 'http://restricted.org/second']
      ...
      expect(select_by_id('fulltextAndSuppl2')['url_fulltext']).to eq ['http://restricted.org/first', 'http://www.fulltext.org/first', 'http://www.fulltext.org/second', 'http://restricted.org/second']
      ...
      expect(select_by_id('fulltextAndSuppl3')['url_fulltext']).to eq ['http://www.fulltext.org/first', 'http://restricted.org/first', 'http://www.fulltext.org/second', 'http://restricted.org/second']
      ...
      expect(select_by_id('fulltextAndSuppl4')['url_fulltext']).to eq ['http://restricted.org/first', 'http://restricted.org/second', 'http://www.fulltext.org/first', 'http://www.fulltext.org/second']
