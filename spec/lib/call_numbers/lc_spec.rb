...
describe CallNumbers::LC do
  ...
  describe 'standard numbers' do
    it 'handles 1 - 3 class letters' do
      expect(CallNumbers::LC.new('P123.23 .M23 2002').klass).to eq 'P'
      expect(CallNumbers::LC.new('PS123.23 .M23 2002').klass).to eq 'PS'
      expect(CallNumbers::LC.new('PSX123.23 .M23 2002').klass).to eq 'PSX'
    end

    it 'parses the class number and decimal' do
      expect(CallNumbers::LC.new('P123.23 .M23 2002').klass_number).to eq '123'
      expect(CallNumbers::LC.new('P123.23 .M23 2002').klass_decimal).to eq '.23'

      expect(CallNumbers::LC.new('P123 .M23 2002').klass_number).to eq '123'
      expect(CallNumbers::LC.new('P123 .M23 2002').klass_decimal).to be_nil
    end

    describe 'doons (dates or other numbers)' do
      it 'parses a doon after the class number/decimal and before the 1st cutter' do
        expect(CallNumbers::LC.new('P123.23 2012 .M23 2002 .M45 V.1').doon1).to match(/^2012/)
      end
    ...
    describe 'cutters' do
      it 'handles 3 possible cutters' do
        expect(CallNumbers::LC.new('P123.23 .M23 V.1 2002').cutter1).to eq '.M23'
        expect(CallNumbers::LC.new('P123.23 .M23 V.1 2002').cutter2).to be_nil
        expect(CallNumbers::LC.new('P123.23 .M23 V.1 2002').cutter3).to be_nil

        expect(CallNumbers::LC.new('P123.23 .M23 .M45 V.1 2002').cutter1).to eq '.M23'
        expect(CallNumbers::LC.new('P123.23 .M23 .M45 V.1 2002').cutter2).to eq '.M45'
        expect(CallNumbers::LC.new('P123.23 .M23 .M45 V.1 2002').cutter3).to be_nil

        expect(CallNumbers::LC.new('P123.23 .M23 .M45 .S32 V.1 2002').cutter1).to eq '.M23'
        expect(CallNumbers::LC.new('P123.23 .M23 .M45 .S32 V.1 2002').cutter2).to eq '.M45'
        expect(CallNumbers::LC.new('P123.23 .M23 .M45 .S32 V.1 2002').cutter3).to eq '.S32'
      end

      it 'handles multi-letter cutters' do
        expect(CallNumbers::LC.new('P123.23 .MS23').cutter1).to eq '.MS23'
        expect(CallNumbers::LC.new('P123.23 .MSA23').cutter1).to eq '.MSA23'
      end

      it 'handles a single letter after the cutter number' do
        expect(CallNumbers::LC.new('P123.23 .MS23A').cutter1).to eq '.MS23A'
      end
      ...
      it 'parses cutters with no space or period' do
        expect(CallNumbers::LC.new('P123M23S').cutter1).to eq 'M23S'
        expect(CallNumbers::LC.new('P123M23SL').cutter1).to eq 'M23SL'
    ...
    describe 'folio' do
      it 'handles folio' do
        expect(CallNumbers::LC.new('P123M23S32 F 123').folio).to eq 'F'
      end

      it 'handles a folio at the end of the string' do
        expect(CallNumbers::LC.new('P123M23S32 F').folio).to eq 'F'
      end

      it 'handles a flat folio at the end of the string' do
        expect(CallNumbers::LC.new('P123M23S32 FF').folio).to eq 'FF'
      end

      it 'does nothing with tons of Fs' do
        expect(CallNumbers::LC.new('P123M23S32 FFFFF').folio).to eq nil
      end
    end

    describe 'the rest of the stuff' do
      it 'puts any other content into the rest attribute' do
        expect(CallNumbers::LC.new('P123.23 .M23 V.1 2002').rest).to eq 'V.1 2002'
        expect(CallNumbers::LC.new('P123.23 2012 .M23 2002 .M45 V.1 2002-2012/GobbldyGoop').rest).to eq 'V.1 2002-2012/GobbldyGoop'
      end
    end
