...
describe 'comparing against a well-known location full of documents generated by solrmarc' do
  ...
  let(:ignored_fields) do
    %w[all_search mhld_display created last_updated format _version_ author_sort callnum_facet_hsim marcbib_xml marcxml fund_facet building_facet collection] + copy_fields
  end
  let(:pending_fields) { %w[reverse_shelfkey shelfkey preferred_barcode item_display date_cataloged] }
  ...
  Dir.glob(File.expand_path('solrmarc_example_docs/*', file_fixture_path)).each do |fixture|
    context "with #{fixture}" do
      ...
      it 'maps building_facet' do
        # Some buildings change display names
