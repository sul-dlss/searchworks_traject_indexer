...
describe 'comparing against a well-known location full of documents generated by solrmarc' do
  ...
  let(:marc_record) do
    MARC::XMLReader.new(StringIO.new(HTTP.get("https://searchworks.stanford.edu/view/#{catkey.sub(/^a/, '')}.marcxml").body.to_s)).to_a.first
  end

  shared_examples 'records match' do
    let(:client) { FolioClient.new url: ENV['OKAPI_URL'], username: ENV['OKAPI_USER'], password: ENV['OKAPI_PASSWORD'] }
    let(:folio_record) do
      if ENV['POSTGRES_URL']
        Traject::FolioPostgresReader.new(nil, 'postgres.url' => ENV['POSTGRES_URL'], 'postgres.sql_filters' => "lower(sul_mod_inventory_storage.f_unaccent(vi.jsonb ->> 'hrid'::text)) = '#{catkey}'").first
      else
    ...
    it 'matches' do
      aggregate_failures "testing response" do
        mapped_fields.each do |key|
          ...
          expect(folio_result[key]).to eq(sirsi_result[key]), "expected #{key} to match \n\nSIRSI:\n#{sirsi_result[key].inspect}\nFOLIO:\n#{folio_result[key].inspect}"
        end
