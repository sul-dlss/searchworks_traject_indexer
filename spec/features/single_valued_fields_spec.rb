...
RSpec.describe 'Single valued fields' do
  ...
  context 'Arabic example' do
    ...
    it {
      expect(result['title_245_search'].first).to eq "Epître sur l'unité et la Trinité : Traité sur l'intellect, Fragment sur l'ame /"
    }
  ...
  context 'Japanese example' do
    ...
    it {
      expect(result['title_245a_search'].first).to eq 'Annotated economic statistics of Japan for postwar years up to 1958 /'
    }
    it {
      expect(result['title_245_search'].first).to eq 'Annotated economic statistics of Japan for postwar years up to 1958 /'
    }
    ...
    it {
      expect(result['title_full_display'].first).to eq 'Annotated economic statistics of Japan for postwar years up to 1958 / The Institute of Economic Research, Hitotusubashi University.'
    }
  end
end
