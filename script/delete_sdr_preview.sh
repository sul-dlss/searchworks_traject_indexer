...
bundle exec traject -c ./lib/traject/config/sdr_delete_config.rb -s purl_fetcher.target="" -s skip_if_catkey=false -s purl_fetcher.first_modified="${LAST_DATE}" -s solr_writer.max_skipped=-1 /dev/null
