...
to_field 'dct_title_s', stanford_mods(:sw_short_title, default: '[Untitled]')
...
to_field 'dct_format_s', mods_xpath('mods:extension[@displayLabel="geo"]//dc:format') do |_record, accumulator|
...
to_field 'dct_format_s' do |record, accumulator, context|
...
to_field 'dct_language_sm', stanford_mods(:sw_language_facet)
to_field 'dct_subject_sm', stanford_mods(:subject_other_search) do |_record, accumulator|
  accumulator.map! { |val| val&.sub(/[\\,;]$/, '')&.strip }
end
to_field 'dct_subject_sm', mods_xpath('mods:subject/mods:topic') do |_record, accumulator|
  accumulator.flatten!
...
to_field 'dc_publisher_sm', stanford_mods(:term_values, %I[origin_info publisher])
to_field 'dct_creator_sm', stanford_mods(:sw_person_authors) do |_record, accumulator|
  accumulator.map! { |str| trim_punctuation_when_preceded_by_two_word_characters_or_some_other_stuff(str) }
end

to_field 'dct_creator_sm', stanford_mods(:sw_corporate_authors) do |_record, accumulator|
  accumulator.map! { |str| trim_punctuation_when_preceded_by_two_word_characters_or_some_other_stuff(str) }
end
...
to_field 'locn_geometry', stanford_mods(:geo_extensions_as_envelope)
to_field 'locn_geometry', stanford_mods(:coordinates_as_envelope)
to_field 'id' do |record, accumulator|
  accumulator << "stanford-#{record.druid}"
end
to_field 'gbl_wxsIdentifier_s' do |record, accumulator|
...
# FIXME: use?
# to_field 'dct_rights_sm' do |record, accumulator|
#   accumulator << record.rights_xml
# end

# TODO: should use license URIs
# to_field 'dct_license_sm' do |record, accumulator|
#   field = record.mods_display.accessCondition.find { |x| x.label == 'License:' }

#   if field
#     accumulator << field.values.map(&:to_s)
#   else
...
to_field 'dct_rightsHolder_sm' do |record, accumulator|
...
# TODO: add gbl_dateRange_drsim support
# see: https://opengeometadata.org/docs/ogm-aardvark/date-range
# to_field 'gbl_dateRange_drsim' do |record, accumulator|
#   subject_dates = record.stanford_mods.subject.temporal.map { |t| Mods::Date.from_element(t) }
#   subject_year_ranges = subject_dates.map { |x| x&.as_range }.compact

#   accumulator << subject_year_ranges
#   accumulator << record.stanford_mods.pub_year_int if accumulator.empty?
# end

to_field 'dct_source_sm' do |record, accumulator|
...
each_record do |_record, context|
  ...
  if context.output_hash['locn_geometry'].present?
    context.output_hash['locn_geometry'] = context.output_hash['locn_geometry'].first
  else
    context.skip!(
...
each_record do |_record, context|
  ...
  logger.debug('geo_aardvark_config.rb') { "Processed #{context.output_hash['id']} (#{t1 - t0}s)" }
end
